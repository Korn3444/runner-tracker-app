    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Your Running Dashboard</h1>
            <p className="text-sm text-slate-500">Monitor your running statistics and logged logs</p>
          </div>
          <Link
          ...
          >
            <Plus className="w-4 h-4" /> Log New Run
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Total Distance</p>
              <h3 className="text-2xl font-bold text-slate-900">{totalDistance.toFixed(2)} km</h3>
            </div>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            <div className="p-3 bg-orange-50 text-orange-600 rounded-xl">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900">{totalDuration} mins</h3>
            </div>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            <div className="p-3 bg-orange-50 text-orange-600 rounded-xl">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Total Workouts</p>
              <h3 className="text-2xl font-bold text-slate-900">{totalRuns} times</h3>
            </div>
          </div>
        </div>

        {/* Run Feed Logs */}
        <h2 className="text-lg font-bold text-slate-900 mb-4">Recent Running History</h2>
        {!runs || runs.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center">
            ...
            <p className="text-slate-600 font-medium mb-1">No runs recorded yet</p>
            <p className="text-slate-400 text-sm mb-4">Get off the couch and start log your first milestone.</p>
            <Link
              href="/dashboard/runs/create"
                  ...
                  <div className="p-5">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                      <div>
                        ...
                        <span className="text-lg font-extrabold text-slate-800">{run.distance_km} <span className="text-xs font-normal text-slate-500">km</span></span>
                      </div>
                      <div>
                        <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">Duration</span>
                        <span className="text-lg font-extrabold text-slate-800">{run.duration_minutes} <span className="text-xs font-normal text-slate-500">mins</span></span>
                      </div>
                    </div>
                    {run.notes && (
